...
BEGIN TRY
    ...
    WITH CleanedStaging AS (
        SELECT
            ...
            TRY_CONVERT(INT, TRY_CONVERT(FLOAT, NULLIF(LTRIM(RTRIM(delivery_date_key)), ''))) AS delivery_date_key,
            ...
            TRY_CONVERT(INT, TRY_CONVERT(FLOAT, NULLIF(LTRIM(RTRIM(estimated_days)), '')))   AS estimated_days,
            TRY_CONVERT(INT, TRY_CONVERT(FLOAT, NULLIF(LTRIM(RTRIM(actual_days)),    '')))   AS actual_days,
    ...
    SELECT
        ...
        -- FLAGS: dikalkulasi dari status_code
...
SELECT
    COUNT(*)                                                AS total_rows,
    SUM(CASE WHEN actual_days IS NOT NULL THEN 1 ELSE 0 END) AS actual_days_filled,
    SUM(CASE WHEN actual_days IS NULL     THEN 1 ELSE 0 END) AS actual_days_null,
    SUM(CASE WHEN delay_days > 0          THEN 1 ELSE 0 END) AS delay_gt_0,
    SUM(CASE WHEN delay_days = 0          THEN 1 ELSE 0 END) AS delay_eq_0,
    SUM(CASE WHEN delay_days IS NULL      THEN 1 ELSE 0 END) AS delay_null,
