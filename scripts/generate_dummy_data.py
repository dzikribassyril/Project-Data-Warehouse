...
# ==========================================
# Lookup Tables (Konsisten di seluruh script)
# ==========================================
...
# ==========================================
# 1. Generate DimDate (Tahun 2025, 365 hari)
# ==========================================
...
for i in range(1, NUM_COURIERS + 1):
    dim_courier_data.append({
        ...
        'vehicle_type':    random.choices(          
            ['Motor', 'Mobil Van', 'Mobil Box', 'Sepeda'],
...
# ==========================================
# 5. Generate DimService (6 baris — tetap)
# ==========================================
services = [
...
for i in range(1, NUM_TRANSACTIONS + 1):
    ...
    volume = l * w_cm * h
    pkg_cat = 'Kecil' if volume < 5000 else 'Sedang' if volume < 50000 else 'Besar'
    dim_package_data.append({
        'package_key':      i,
        'package_id':       f'PKG-{i:07d}',
        'package_type':     random.choice(['Box', 'Plastik', 'Dokumen']),
        'package_category': pkg_cat,         
        'weight':           w,
...
for i in range(1, NUM_TRANSACTIONS + 1):
    ...
    if method == 'Transfer Bank':
        ...
        channel = random.choice(EWALLET_CHANNELS)
        bank = None
    else:
        channel = 'COD'
        bank = None
...
# ==========================================
# 8. Generate DimShipmentStatus (8 baris)
# ==========================================
statuses = [
...
for i in range(1, NUM_ROUTES + 1):
    ...
    origin_region = city_region_map[origin]
    dest_region   = city_region_map[destination]
    if origin_region == dest_region:
        route_type = 'Intra-Region'
    elif origin_region in ['Jabodetabek', 'Jawa'] and dest_region in ['Jabodetabek', 'Jawa']:
...
# ==========================================
# 10. Generate FactDeliveryPerformance
# ==========================================

IN_PROGRESS_STATUS_KEYS = [1, 2, 3, 4]  # PICKUP, INTRANSIT, ONSORTIR, OUTDELIVERY
TERMINAL_STATUS_KEYS    = [5, 6, 7, 8]  # DELIVERED, FAILED, RETURNED, CANCELLED
...
for i in range(1, NUM_TRANSACTIONS + 1):
    ...
    if outer == 'delivered':
        status_key = 5
    elif outer == 'in_progress':
        status_key = random.choice(IN_PROGRESS_STATUS_KEYS)
    elif outer == 'failed':
        status_key = 6
    ...
    if i % 10_000 == 0:
        pct = i * 100 // NUM_TRANSACTIONS
        print(f"  → {i:,}/{NUM_TRANSACTIONS:,} baris ({pct}%)")

pd.DataFrame(fact_data).to_csv('dataset/FactDeliveryPerformance.csv', index=False)
...
for f in sorted(os.listdir('dataset')):
    path = f'dataset/{f}'
    df   = pd.read_csv(path)
    print(f"  {f:<40} {len(df):>8,} baris  |  {df.shape[1]} kolom")